...
void ascendingSwap(int index1, int index2,
                   int *ar)                                           //swap two values such that they appear in ascending order in the array
{
...
void decendingSwap(int index1, int index2,
                   int *ar)                                           //swap two values such that they appear in decending order in the array
{
...
void bitonicSortFromBitonicSequence(int startIndex, int lastIndex, int dir,
                                    int *ar)     //form a increaseing or decreasing array when a bitonic input is given to the function
{
    if (dir == 1) {
...
void bitonicSequenceGenerator(int startIndex, int lastIndex,
                              int *ar)                         //generate a bitonic sequence  from a a random order
{
    int noOfElements = lastIndex - startIndex + 1;
...
#pragma omp parallel for                                                         //parallel implementation results in most performance gains here
        for (int i = 0; i < noOfElements; i = i + j) {
            if (((i / j) % 2) ==
                0)                                                               //extra computation results in drastically lower performance ,need to fix
            {
...
{
    omp_set_dynamic(
            0);                                                                          //disabled so that the os doesnt override the thread settings
    ...
    int n;
    cout
            << "enter the number of elements to be sorted (number should be in the order of 2^n)";     //try to fix this issue
    cin >> n;
    int *ar = new int[n];
    for (int i = 0; i < n; i++) {
        cin >> ar[i];
